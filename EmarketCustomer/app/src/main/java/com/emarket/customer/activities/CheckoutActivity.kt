...
class CheckoutActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val accDiscount = UserViewModel(application).user!!.accDiscount
        ...
        discountCheck.setOnCheckedChangeListener { _, isChecked ->
            ...
            discountView.text = if (isChecked) getString(R.string.template_price, maxOf(transaction.total - accDiscount, 0.0) ) else ""
        }

        confirmButton.setOnClickListener {
            transaction.total = transaction.products.fold(0.0) { sum, product -> sum + product.price * product.qnt}
            transaction.discounted = if (discountCheck.isChecked) minOf(transaction.total, accDiscount) else 0.0
